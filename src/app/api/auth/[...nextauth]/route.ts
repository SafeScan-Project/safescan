...
export const authOptions = {
    ...
    callbacks: {
        async jwt({
            ...
            // When calling useSession().update(...), merge the updated fields into JWT.
            if (trigger === "update" && session) {
                if (typeof session.displayName !== "undefined") {
                    token.displayName = session.displayName;
                }
                if (typeof session.profilePicture !== "undefined") {
                    token.profilePicture = session.profilePicture;
                }
                if (typeof session.bio !== "undefined") {
